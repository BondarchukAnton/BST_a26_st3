...
def search_at_point(current_point):
    ...
    while time.time() - t0 < SEARCH_DURATION and not target_found and not should_stop:
        # Log ArUco markers every 3 seconds
        ...
        if frame is not None:
            ...
            if run_yolo_on_frame(frame):
                ...
                print(f"[DETECT] синий LED 3 сек...")
                led_error = None
                try:
                    import rclpy
                    ...
                    if client.wait_for_service(timeout_sec=1.0):
                        ...
                        rclpy.spin_until_future_complete(drone.node, future, timeout_sec=2.0)
                        time.sleep(3)
...
def run():
    ...
    try:
        ...
        print(f"[TAKEOFF] команда отправлена ({{time.time() - t0:.1f}} сек), ожидание 10 сек...")
        time.sleep(10)
        ...
        if target_found:
            ...
            print(f"[DONE] полёт к метке 50 (0.4, 0.4)...")
            drone.control.navigate(x=0.4, y=0.4, z=ALTITUDE, yaw=0.0, speed=SPEED,
                                    frame_id="aruco_map", auto_arm=False)
            time.sleep(5)
            print(f"[DONE] захват метки 50, висение...")
            while True:
                drone.control.navigate(x=0.0, y=0.0, z=ALTITUDE, yaw=0.0, speed=0.3,
                                        frame_id="aruco_50", auto_arm=False)
                try:
                    t = drone.control.get_telemetry(frame_id="aruco_50")
                    if t is not None and t.x is not None and t.x == t.x:
                        print(f"[DONE] метка 50: dx={{t.x:+.2f}} dy={{t.y:+.2f}} dz={{t.z:+.2f}}")
                    else:
                        print(f"[DONE] метка 50 не видна")
                except Exception:
                    pass
                time.sleep(1.0)
