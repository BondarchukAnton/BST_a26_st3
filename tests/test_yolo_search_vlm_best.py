...
def run_yolo_on_frame(frame):
    global last_annotated_frame, frame_counter
    frame_counter += 1
...
def search_at_point(current_point):
    ...
    wx, wy = current_point
    print(f"[SEARCH] поиск у точки ({{wx:.3f}}, {{wy:.3f}}), стабилизация 1.5 сек...")
    time.sleep(1.5)
    print(f"[SEARCH] запуск YOLO, длительность {{SEARCH_DURATION:.0f}} сек...")
    t0 = time.time()
    frames_ok = 0
    last_report = 0
    while time.time() - t0 < SEARCH_DURATION and not target_found and not should_stop:
        ...
        if frame is not None:
            ...
            if run_yolo_on_frame(frame):
                ...
                # Сохраняем аннотированный кадр с подписью VLM внизу
                if last_annotated_frame is not None:
                    result_img = last_annotated_frame.copy()
                    h, w = result_img.shape[:2]
                    lines = []
                    if vlm_result_cache:
                        lines.append(f"VLM: che={{vlm_result_cache.get('cheburashka')}} "
                                     f"conf={{vlm_result_cache.get('confidence')}} "
                                     f"dir={{vlm_result_cache.get('direction')}}")
                        lines.append(f"{{vlm_result_cache.get('summary', '')}}")
                    else:
                        lines.append("VLM: no response")
                    y0 = h - 6 - len(lines) * 15
                    for i, line in enumerate(lines):
                        y = y0 + i * 15
                        cv2.putText(result_img, line, (6, y),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 255, 0), 1)
                    cv2.imwrite("/tmp/cheburashka_detected.jpg", result_img)
                    print("[SAVE] /tmp/cheburashka_detected.jpg (YOLO + VLM)")
...
def run():
    ...
    yolo_model.overrides['imgsz'] = 320 ##########################################################
    print(f"[INIT] готово ({{time.time() - t0:.1f}} сек)")
...
def main():
    ...
    try:
        ...
        print(f"[HOST] скачивание снимка...")
        sftp = ssh.open_sftp()
        try:
            sftp.get("/tmp/cheburashka_detected.jpg", "cheburashka_detected.jpg")
            print(f"[HOST] снимок сохранён: {os.path.abspath('cheburashka_detected.jpg')}")
        except Exception:
            print(f"[HOST] снимок не найден на дроне (объект не обнаружен)")
        finally:
            sftp.close()
